...
import { CommandCodeAcpAgent } from "./adapter.ts";
...
async function main() {
  const agentImpl = new CommandCodeAcpAgent();
  // Single-flight model discovery; safe to kick off early.
...
main().catch((err) => {
  console.error("[commandcode-acp] fatal:", err);
  process.exit(1);
});
